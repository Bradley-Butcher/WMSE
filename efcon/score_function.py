...
def weighted_mse(data: pd.DataFrame, child: int, parents: np.ndarray, regret_penalty: bool = False, split: bool = False):
    ...
    if regret_penalty:
        ...
        # params = p_ygxs.shape[1]
        #penalty = ((params * np.log(N)) / score) / data.shape[0]
        inv_hx = 1/(-np.sum(np.log(p_xs) * p_xs))
        inv_hy = 1/(-np.sum(np.log(p_y) * p_y))
        penalty = (params * inv_hx * inv_hy) 
